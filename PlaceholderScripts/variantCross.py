...
def create_cross_geometry(start_level , end_level) : 
    ...
    def create_diagonal_wall_old(doc , partition_lines , first_random , second_random , p) : 
    ...
    for i in range(len(partition_lines_x_long)) : 
        ...
        if i > 1 :
            if first_random_list_x_long[i - 1] == 0 and first_random_list_x_long[i - 2] == 0 : 
                random_number = 1
        if i == len(partition_lines_x_long) - 1 or i == 0 : 
            random_number = 1
    ...
    for i in range(len(partition_lines_x_short)) : 
        ...
        if i > 1 :
            if first_random_list_x_short[i - 1] == 0 and first_random_list_x_short[i - 2] == 0 : 
                random_number = 1
        if i == len(partition_lines_x_short) - 1 or i == 0 : 
            random_number = 1

        first_random_list_x_short.append(random_number)

    spatial_buffer = IN[11]
    source_counter = 0
    ...
    y_i = convert_to_meter(0.)
    y_iplus1 = convert_to_meter(y_corridor-CORR_WIDTH/2.)
    ...
    if INCLUDE_BOTTLENECK and len(partition_lines_x_short) > 2 : 
    ...
    file_name = f"{IN[10]}/partition_walls_cross_{start_level.Name}.txt"
